...
class Greenergy_Post_Views
{
    ...
    private function __construct()
    {
        ...
        add_action('save_post_companies', [$this, 'initialize_sort_key'], 10, 3);
        ...
        // Admin columns for Companies
        add_filter('manage_companies_posts_columns', [$this, 'add_news_views_columns']);
        add_action('manage_companies_posts_custom_column', [$this, 'render_news_views_columns'], 10, 2);
        add_filter('manage_edit-companies_sortable_columns', [$this, 'make_news_views_columns_sortable']);
    ...
    {
        if (! is_singular(['news', 'post', 'jobs', 'companies'])) {
            return;
        }
    ...
    public function initialize_sort_key(int $post_id, $post, bool $update): void
    {
        ...
        if (! in_array($post->post_type, ['news', 'post', 'jobs', 'companies'], true)) {
            return;
        }
    ...
    {
        if (! is_admin() || ! $query->is_main_query() || ! in_array($query->get('post_type'), ['news', 'jobs', 'post', 'companies'])) {
            return;
        }
